...
def r_squared(y, estimated):
    ...
    """
    import numpy as np    
    numer = np.mean((np.array(y)-np.array(estimated))**2)
    denom = np.mean((np.array(y)-np.mean(y))**2)
...
def evaluate_models_on_training(x, y, models):
    ...
    for mod in models:
        est = np.polyval(mod,x)
        r2 = r_squared(y,est)
        pylab.plot(x,est,'r-',label='Estimates')
        pylab.title(('Degree: ',str(len(mod)-1),' R2: ',str(round(r2,5))))
